    ...
    <Collapsible.Root className="data-details">
      ...
      <Collapsible.Content className="data-details__content">
        <div className="detail-grid">
          ...
          <section><h3>자동화</h3><p className="detail-status"><RefreshCw aria-hidden="true" />{automationAssessment.state === 'ok' ? '최근 자동화 정상' : automationAssessment.label}</p><p>업데이트: {formatDateTime(automation?.updatedAt)}</p>{automation?.details?.length ? <ul>{automation.details.map((detail) => <li key={detail}>{detail}</li>)}</ul> : null}<a href="https://github.com/SonChangGi/dram-price/actions/workflows/update-data.yml" target="_blank" rel="noreferrer">GitHub Actions 열기<ArrowUpRight aria-hidden="true" /></a></section>
        </div>
