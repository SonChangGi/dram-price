...
class ScheduledRecoveryTests(unittest.TestCase):
    ...
    def test_each_daytime_slot_skips_after_complete_price_day_success(self):
    ...
    def test_night_slots_skip_after_the_same_price_day_is_complete(self):
    ...
    def test_valid_morning_session_is_complete_without_forcing_an_evening_session(self):
        ...
        self.rows.pop()
        self.assertTrue(self.decide().should_collect)

    def test_source_session_time_gate_remains_optional_only(self):
        for row in self.rows:
            row["source_last_update"]["time"] = "11:00"
        decision = self.decide(minimum_source_time="17:50")
        self.assertTrue(decision.should_collect)
        self.assertEqual(decision.reason, "missing-required-source-session")
        for row in self.rows:
            row["source_last_update"]["time"] = "17:50"
        self.assertFalse(self.decide(minimum_source_time="17:50").should_collect)
    ...
    def test_recovery_force_preserves_target_date_without_adding_a_session_time_gate(self):
    ...
    def test_wrong_or_missing_source_timezone_does_not_count_as_final_session(self):
        ...
        self.assertEqual(self.decide(minimum_source_time="17:50").daily_observation_count, 0)
