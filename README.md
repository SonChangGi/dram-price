...
- `.github/workflows/update-data.yml` is manual-only after the multi-repo rollback. A reviewed `workflow_dispatch` run forces collection, re-checks the requested target date before tests/commit/deploy, validates the public data publication floor, commits `data/` only when safe data changes exist, and deploys the refreshed static site to GitHub Pages in the same workflow. Legacy scheduled guards remain in the script for recoverability, but no cron schedule is active.
