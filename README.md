...
| KST 예약 시각 | 대상 가격일 | 동작 |
| --- | --- | --- |
| 월–금 13:15, 15:15, 19:30 | 해당 거래일 | 오전에 성공해도 다시 수집해 더 늦은 세션을 반영 |
| 월–금 22:30 | 해당 거래일 | 누락·실패 또는 마지막 세션 미확보 시 재시도 |
| 화–토 01:30, 04:30 | 전날 거래일 | 원문 또는 보존한 스냅샷으로 전날을 복구 |

- `prices.json`과 `series.json`은 제품·원문 날짜별 관측을 계속 누적한다. 같은 날짜에는 더 늦은 `source_last_update`의 세션 평균을 사용하며, 늦게 도착한 오전 응답이 저녁 값을 덮을 수 없다. 고정가·주간·월간 자료를 가짜 일별 값으로 늘리지 않는다.
...
- 실행 생성 시각과 예약 슬롯으로 가격일을 한 번 확정한다. 대기 중 자정이 지나도 대상일이 바뀌지 않는다. 야간 복구 완료는 해당 날짜의 7제품 모두 마지막 세션 구간(GMT+8 17:50 이후) 관측을 확보했는지 검사한다. 화면의 값은 공식 종가로 재명명하지 않고 원문의 세션 평균을 유지한다. [원문 세션 안내](https://www.dramexchange.com/Service/Spot_Price_Notice)
