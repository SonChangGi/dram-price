...
- `.github/workflows/update-data.yml` runs scheduled public-source refreshes at 09:00 KST daily, with a 06:00 KST next-day retry that verifies yesterday when TrendForce publishes late. A reviewed `workflow_dispatch` run still forces collection, re-checks the requested target date before tests/commit/deploy, validates the public data publication floor, commits `data/` only when safe data changes exist, and deploys the refreshed static site to GitHub Pages in the same workflow. Scheduled provider outages remain soft-fail warnings and never publish partial data.
